...
export const getConsentForSellPropertySchema = (lang: string) =>
  z.object({
    ...
    passportInternation: z.string().min(3, {
      message: lang === 'ua' ? 'Введіть номер та серію закордонного паспорта мін 3 літери' : 'Passport number is required',
    }),
    ...
    marriageCertRecordNumber: z.string().min(1, { message: lang === 'ua' ? 'Введіть номер актового запису' : 'Record number is required' }),
