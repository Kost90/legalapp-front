...
export const getVehicleSchema = (lang: string) =>
  z.object({
    ...
    vehicleLicensePlate: z.string().min(4, lang === 'ua' ? 'Введіть номерний знак, мінімальна кількість 4' : 'License plate is required'),
    vehicleType: z.string().min(3, lang === 'ua' ? 'Введіть тип ТЗ' : 'Vehicle type is required'),
    vehicleRegistrationCertificate: z
      .string()
      .min(3, lang === 'ua' ? 'Введіть номер свідоцтва' : 'Registration certificate number is required'),
